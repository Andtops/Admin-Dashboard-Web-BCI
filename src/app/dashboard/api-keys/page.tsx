...
export default function APIKeysPage() {
  ...
  const [editSelectAllEssential, setEditSelectAllEssential] = useState(false);
  ...
  const { updateApiKey, loading: isUpdatingKey, error: updateError } = useUpdateApiKey();
      ...
      permissions: [
        {
          name: "search:read",
          label: "Read Search",
          description: "Access search functionality and results"
        },
        {
          name: "search:write",
          label: "Write Search",
        ...
        },
        {
          name: "search:admin",
          label: "Admin Search",
          description: "Full search administration and management"
        }
      ]
    },
    webhooks: {
      ...
      permissions: [
        {
          name: "admins:read",
          label: "Read Admins",
          description: "View admin accounts and roles"
        },
        {
          name: "admins:write",
          label: "Write Admins",
          description: "Create and modify admin accounts"
        },
        {
          name: "admins:delete",
          label: "Delete Admins",
    ...
    logs: {
      ...
      permissions: [
        ...
        },
        {
          name: "logs:write",
          label: "Write Logs",
          description: "Create and modify log entries"
        }
      ]
    },
    reviews: {
      ...
      permissions: [
        ...
        },
        {
          name: "reviews:write",
          label: "Write Reviews",
          description: "Create and moderate product reviews"
        },
        {
          name: "reviews:delete",
          label: "Delete Reviews",
          description: "Remove product reviews"
        }
      ]
    },
    orders: {
  ...
  const getEssentialPermissions = () => {
    // Get commonly used permissions from multiple categories
    const essentialCategories = ['collections', 'products', 'quotations', 'users', 'analytics'] as const;
    return essentialCategories.flatMap(categoryKey =>
  ...
  const handleEditApiKey = (apiKey: any) => {
    ...
    // Check if all essential permissions are selected
    const essentialPerms = getEssentialPermissions();
    const hasAllEssential = essentialPerms.every(p => apiKey.permissions.includes(p));
    setEditSelectAllEssential(hasAllEssential);

    setShowEditDialog(true);
  };

  const handleEditSelectAllEssential = (checked: boolean) => {
    setEditSelectAllEssential(checked);
    const essentialPerms = getEssentialPermissions();

    if (checked) {
      // Add all essential permissions that aren't already selected
      const newPermissions = [...new Set([...editKeyPermissions, ...essentialPerms])];
      setEditKeyPermissions(newPermissions);
    } else {
      // Remove all essential permissions
      const filteredPermissions = editKeyPermissions.filter(p => !essentialPerms.includes(p));
      setEditKeyPermissions(filteredPermissions);
    }
  };

  const handleEditPermissionChange = (permissionName: string, checked: boolean) => {
    if (checked) {
      setEditKeyPermissions([...editKeyPermissions, permissionName]);
    } else {
      setEditKeyPermissions(editKeyPermissions.filter(p => p !== permissionName));
    }

    // Update "Select All Essential" state
    const essentialPerms = getEssentialPermissions();
    const hasAllEssential = essentialPerms.every(p =>
      checked && permissionName === p ? true : editKeyPermissions.includes(p)
    );
    setEditSelectAllEssential(hasAllEssential);
  };

  const handleUpdateApiKey = async () => {
    if (!admin || !editingApiKey || !editKeyName.trim()) return;
    ...
    try {
      ...
      // Update the selected API key if it's the one being edited
      if (selectedApiKey?.id === editingApiKey.id) {
        setShowDetailsDialog(false);
      }
    } catch (error) {
  ...
  const resetEditForm = () => {
    ...
    setEditSelectAllEssential(false);
  };
            ...
            <div className="flex items-center gap-2">
                            ...
                            .map(([categoryKey, category]) => {
                              ...
                              return (
                                <div key={categoryKey} className="border rounded-lg overflow-hidden">
                                  {/* Category Header with Theme Colors */}
                                  ...
                                  {/* Permission List with Theme Colors */}
                                  <div className="p-4 bg-card">
                                    <div className="space-y-3">
                                      {filteredPermissions.map((permission) => (
                                        <label key={permission.name} className="flex items-start gap-3 p-3 rounded-md hover:bg-accent hover:text-accent-foreground cursor-pointer transition-colors">
                                          <input
                                            type="checkbox"
                                            checked={newKeyPermissions.includes(permission.name)}
                                            onChange={(e) => handlePermissionChange(permission.name, e.target.checked)}
                                            className="mt-0.5 w-4 h-4 text-primary border-input rounded focus:ring-primary focus:ring-2"
                                          />
              ...
              >
                ...
                >
                  <ModalHeader>
                    <ModalTitle className="flex items-center gap-2">
                      {isUpdatingKey && <Loader2 className="h-4 w-4 animate-spin" />}
                      <Edit3 className="h-5 w-5" />
                      Edit API Key
                    </ModalTitle>
                    <ModalDescription>
                      Update the name and permissions for "{editingApiKey?.name}"
                    </ModalDescription>
                  </ModalHeader>
                  ...
                  <div className="flex-1 overflow-y-auto px-8 py-6">
                    <div className="space-y-10">
                      ...
                      <FormSection
                        title="API Permissions"
                        description="Update access levels and permissions for your API key. Search and select permissions by category."
                      >
                        {/* Permission Search */}
                        <div className="mb-6">
                          <div className="relative">
                        ...
                        {/* Single Column Permission Categories */}
                        <div className="space-y-8 max-h-96 overflow-y-auto pr-2">
                          {Object.entries(permissionCategories)
                            .filter(([_, category]) => {
                              if (!editPermissionSearchTerm) return true;
                              const searchLower = editPermissionSearchTerm.toLowerCase();
                              return (
                                category.title.toLowerCase().includes(searchLower) ||
                                category.description.toLowerCase().includes(searchLower) ||
                                category.permissions.some(p =>
                                      ...
                                      <div key={permission.name} className="group bg-white border border-blue-200 rounded-lg p-4 hover:shadow-md hover:border-blue-300 transition-all duration-200">
                                        <div className="flex items-start space-x-3">
                                          <input
                                            ...
                                            checked={editKeyPermissions.includes(permission.name)}
                                            onChange={(e) => handleEditPermissionChange(permission.name, e.target.checked)}
                                            className="mt-1 w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500 focus:ring-2"
                                          />
                                          <div className="flex-1 min-w-0">
                                            <Label htmlFor={`edit-${permission.name}`} className="text-sm font-semibold text-blue-900 cursor-pointer group-hover:text-blue-700 transition-colors">
                                              {permission.label}
                                            </Label>
                        ...
                        {/* Selected Permissions Summary */}
                        {editKeyPermissions.length > 0 && (
                          <div className="mt-6 p-4 bg-gray-50 border border-gray-200 rounded-xl">
                            <h5 className="font-medium text-gray-900 mb-3">Selected Permissions ({editKeyPermissions.length})</h5>
                            <div className="flex flex-wrap gap-2">
                              {editKeyPermissions.map((permission) => (
                                <div key={permission} className="inline-flex items-center gap-1 px-3 py-1 bg-white border border-gray-300 rounded-full text-xs font-medium text-gray-700">
                                  <Shield className="h-3 w-3" />
                                  {permission}
                                </div>
                    ...
                    <div className="flex items-center justify-between w-full">
                      <div className="text-sm text-muted-foreground">
                        {editKeyPermissions.length > 0 ? (
                          <span className="flex items-center gap-1">
                            <Shield className="h-4 w-4" />
                            {editKeyPermissions.length} permission{editKeyPermissions.length !== 1 ? 's' : ''} selected
                          </span>
                        ) : (
                          "No permissions selected"
                        )}
                      ...
                      <div className="flex gap-3">
                        ...
                        <Button
                          ...
                          disabled={!editKeyName.trim() || isUpdatingKey}
                          className="px-6 bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white shadow-lg hover:shadow-xl transition-all duration-200"
                        >
                          {isUpdatingKey ? (
                            <>
